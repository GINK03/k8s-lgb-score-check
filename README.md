...
## 機械学習の最小粒度としてのDocker, Dockerのデプロイ先としてのk8s
 ...
 GCPではコンテナを使ったデプロイメントサービスはKubernetes Engineがデフォルトであり、Web UIやCUIでのヘルプも充実してきました。  
 
 ローリングリリースが簡単にできたり、Docker Fileやコンテナが適切に受け渡しが行われれば、デプロイまでの時間的労力的消耗を最小化できたりします。  
...
予想システムは映画.comさまのコーパスを利用して、LightGBMでテキストコーパスから予想を行います。学習と評価に使ったスクリプトとコーパスはこちらになります。  
